...
function sendRequest(name, phone, address, goods, sum) {
    ...
    for (let i = 0; i < countOfGoods; i += 1) {
        ...
        let tc = {title : '', count : 0};
        tc.title = goods[i].title;
        tc.count = goods[i].count;
        data.goods.push(tc);
    }

//    data.order.address = address;
    data.order.address = 'ул. ' + address.street;
    data.order.address += ', дом ' + address.house;
    data.order.address += ', ' + address.entrance + ' подъезд';
    data.order.address += ', ' + address.floor + ' этаж';
    data.order.address += ', кв. ' + address.flat;

//    data.order.sum = name + phone + address + goods + sum;
    data.order.sum = sum;
    data.client = name + ' ' + phone; //'Иван';
